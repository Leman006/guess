      ...
      <div className="head-wrapper w-[1660px] mx-auto flex justify-between h-[71px] items-center">
        <div className="head-left flex items-center">
          <div className="logo mr-[40px]">
            <Link to={'/'} className='text-[45px] font-[500] font-[Open_Sans]'>GUESS</Link>
          </div>
          <div className="menu text-[#939090] w-[320px] flex justify-between">
            <NavLink to={'/women'} className={({ isActive }) =>
              `text-base font-normal tracking-[0.0625rem] leading-[2.1875rem] transition duration-300 ${isActive ? 'text-black' : 'hover:text-[#1c1b1b]'}`
            }>WOMEN</NavLink>
        ...
        <div className="head-right flex items-center gap-2">
          {!user ? (
            ...
            <div className="text-[14px] font-[500] mr-[15px]">
              Hi {userName}
            </div>
          )}

          <div className='relative h-[74px] border-b-4 border-transparent hover:border-black transition duration-200'>
            <span ref={buttonRef} onClick={() => setIsOpen((prev) => !prev)} className="px-[2px] cursor-pointer">
              ...
              <div ref={modalRef} className="absolute right-0 bg-white shadow-lg border border-black z-50">
                {user ? (
                  <div className="p-[40px] w-[380px]">
                    {/* Header with Circle Initials and Welcome */}
                    <div className="flex items-center mb-6">
                      <div className="bg-[#3b3b3b] text-white rounded-full w-12 h-12 flex items-center justify-center text-lg font-medium mr-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <Link className="flex items-center gap-2 mb-[14px] hover:underline">
                        <img className='w-[16px] h-[16px] mr-[12px]' src="/icons/card-icon.svg" alt="" />
                        <span className='relative'>
                          Personal data
                          <span className='absolute top-[-6px] right-[-10px] w-[8px] h-[8px] bg-red-600 rounded-full'></span>
                        </span>
                      </Link>
                      <Link className="flex items-center gap-2 mb-[14px] hover:underline">
                    ...
                    <button onClick={handleLogout} className="w-full mt-6 border border-black py-[10px] text-center font-[500] hover:bg-black hover:text-white transition duration-200">
                      Logout
                    </button>
                  </div>
                ) : (
